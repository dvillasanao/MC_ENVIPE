---
title: "Tendencias temporales de las personas que declararon cambiar de residencia como medida de protección contra la delincuencia"
...
La ENVIPE utiliza un diseño probabilístico, estratificado, por conglomerados y en múltiples etapas. Sus características son:
...
-   34 áreas urbanas de interés, con la Ciudad de México dividida en cuatro regiones geográficas (Norte, sur, oriente y poniente).

### Marco muestral

El diseño de la muestra para la ENVIPE se caracteriza por ser **probabilístico**; a la vez, el diseño es **trietápico**, **estratificado** y por **conglomerados**, donde la unidad última de selección es la vivienda y las unidades de observación son los hogares y las personas de 18 años cumplidos o más.
...
<figure>
![](../Output/Estratificación.png){width=70% style="display: block; margin: auto;"}
...
Se formaron cuatro estratos sociodemográficos en los que se agruparon todas las UPM del país, esta estratificación considera las características sociodemográficas de los habitantes de las viviendas, así como las características físicas y el equipamiento de las mismas, expresadas por medio de 34 indicadores* construidos con información del Censo de Población y Vivienda 2010, para lo cual se emplearon métodos estadísticos multivariados.   


::: {style="height:350px; overflow:auto;"}
```{r, echo = FALSE}
require(gt)
tabla <- read_xlsx(path = paste0(here::here(), "/Bases/Tablas.xlsx"))

tabla %>%
 group_by(Grupo) %>% 
 gt() %>% 
  tab_header(
             title = "Indicadores empleados en la estratificación de la muestra maestra",
             subtitle = "ENVIPE") %>% 
  ...
  tab_source_note(
                  source_note = md("Fuente: INEGI. Encuesta Nacional de Victimización y Percepción sobre Seguridad Pública.")) %>% 
   as_raw_html() 
...
A continuación se analizan las tendencias temporales (2011-2023) de las personas que declararon cambiar su residencia como medida de protección contra la delincuencia. El análisis abarca datos por individuo, integrantes del hogar, región y dominio.    

> `AP4_11_10 4.11` Durante 202x, para protegerse de la delincuencia, ¿en este hogar se realizó algún tipo de medida como… cambiarse de vivienda o lugar de residencia?
...
Region %>% 
 gt() %>%
  tab_header(title = "Durante 2023, para protegerse de la delincuencia, ¿en este hogar se realizó algún tipo de medida como… cambiarse de vivienda o lugar de residencia?",
             subtitle = "Personas que cambiaron de vivienda o lugar de residencia por Región") %>%
   fmt_integer(columns = names(Region)[2:15], 
               sep_mark = " ") %>%
     tab_options(heading.title.font.size = 14, 
                 heading.subtitle.font.size = 12,
                 table.font.names = "Century Gothic",
...
Dominio %>% 
 gt() %>%
  tab_header(title = "Durante 2023, para protegerse de la delincuencia, ¿en este hogar se realizó algún tipo de medida como… cambiarse de vivienda o lugar de residencia?",
             subtitle = "Personas que cambiaron de vivienda o lugar de residencia por Dominio") %>%
   fmt_integer(columns = names(Dominio)[2:15], 
               sep_mark = " ") %>%
     tab_options(heading.title.font.size = 14, 
                 heading.subtitle.font.size = 12,
                 table.font.names = "Century Gothic",
...
Hogares %>% 
 gt() %>%
  tab_header(title = "Durante 2023, para protegerse de la delincuencia, ¿en este hogar se realizó algún tipo de medida como… cambiarse de vivienda o lugar de residencia?",
   ...
   fmt_integer(columns = names(Hogares)[2:7], 
               sep_mark = " ") %>%
    fmt_number(columns = names(Hogares)[8:10], 
                decimals = 4) %>%
     tab_options(heading.title.font.size = 14, 
                 heading.subtitle.font.size = 12,
                 table.font.names = "Century Gothic",
      ...
      tab_style(style = "vertical-align:middle; font-weight: bold",
       ...
       cols_width(starts_with("CI") ~ px(80),
                  everything() ~ px(80)) %>%
       tab_footnote(footnote = "Fuente: INEGI. Encuesta Nacional de Victimización y Percepción sobre Seguridad Pública, 2010 - 2023.") %>%
        as_raw_html() 
```
...
Region_Hogar %>% 
 gt() %>%
  tab_header(title = "Durante 2023, para protegerse de la delincuencia, ¿en este hogar se realizó algún tipo de medida como… cambiarse de vivienda o lugar de residencia?",
             subtitle = "Total de personas que cambiaron de vivienda o lugar de residencia por Región") %>%
   fmt_integer(columns = names(Region_Hogar)[2:15], 
               sep_mark = " ") %>%
     tab_options(heading.title.font.size = 14, 
                 heading.subtitle.font.size = 12,
                 table.font.names = "Century Gothic",
...
Dominio_Hogar %>% 
 gt() %>%
  tab_header(title = "Durante 2023, para protegerse de la delincuencia, ¿en este hogar se realizó algún tipo de medida como… cambiarse de vivienda o lugar de residencia?",
             subtitle = "Total de personas que cambiaron de vivienda o lugar de residencia por Dominio") %>%
   fmt_integer(columns = names(Dominio_Hogar)[2:15], 
               sep_mark = " ") %>%
     tab_options(heading.title.font.size = 14, 
                 heading.subtitle.font.size = 12,
                 table.font.names = "Century Gothic",
...
Encuesta Nacional de Victimización y Percepción sobre Seguridad Pública (2024): Diseño muestral. Instituto Nacional de Estadística y Geografía (INEGI). 37 p. Recuperado de https://www.inegi.org.mx/contenidos/productos/prod_serv/contenidos/espanol/bvinegi/productos/nueva_estruc/889463920014.pdf
